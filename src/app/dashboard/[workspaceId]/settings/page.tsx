...
import { enableFirstView, getFirstView } from "@/app/action/workspace";
...
const SettingPage = (props: Props) => {
  ...
  const [firstView, setFristView] = useState<undefined | boolean>(undefined);

  useEffect(() => {
    if (firstView !== undefined) return;
    const fetchData = async () => {
      const response = await getFirstView();
      if (response.status === 200) setFristView(response?.data?.firstView);
    };
    fetchData();
  }, [firstView]);
  ...
  return (
    <div>
      ...
      <h2 className="flex items-center gap-x-3 mt-4 text-md">
      ...
      <Label className="flex items-center gap-x-3 mt-4 text-md">
        ...
        <Switch
          ...
          checked={firstView}
          onClick={() => setFristView(!firstView)}
        />
      </Label>
    </div>
  );
